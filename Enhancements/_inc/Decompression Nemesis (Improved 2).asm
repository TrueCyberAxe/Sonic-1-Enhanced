...
NemDec:
        ...
        lea     NemPCD_WriteRowToVDP(pc),a3 ;  8 a3  = jump address for pixel row writes
...
NemDec_Main:
        ...
        lea     NemPCD_WriteRowToVDP_XOR-NemPCD_WriteRowToVDP(a3),a3 ;  8 yes, so in Mode 1 (each row XOR'd with last, only changes recorded)
@0:
        lsl.w   #3,d3                   ; 6 + 2*3 = 12 -.
        subq.w  #1,d3                   ;            4 -' d3 = #/patterns * 8 (= number of rows to plot), minus 1

        bsr.w   NemDec_BuildCodeTable                 ; 18 d0-d2/d5/a5 build opcode-to-count/color lookup table
...
NemPCD_NewRow: ; this is an entry point for compatibility with code in sonic.asm
...
loc_1502:
        ...
        dbra    d3,NemPCD_NewRow_1            ; 10/12 have all 8-pixel rows been written ? if yes, branch
