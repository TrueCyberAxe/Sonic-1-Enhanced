...
eval "diff -ibuwIBEZ <(grep -E '^\S{8} \S' $FILE1 | $FILTER)  <(grep -E '^\S{8} \S' $FILE2 | $FILTER)" | sort | uniq -s1 -w36 -c | grep -E '^\s+1\s+[+-][0-9]' | cut -c10- > diff.lst
# | sort | uniq -w36 -c | grep -E '^\s+1\s+[+-]' > diff.lst
# | grep -E '^[+-]'
# | grep -E '^[ +-]' | sort
# | uniq -s1 -w36 -c | grep -E '^\s+1\s+[+-]'
# | cut -c10- | sort > diff.lst
